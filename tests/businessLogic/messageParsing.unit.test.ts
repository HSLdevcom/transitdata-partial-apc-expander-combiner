...
} from "../../src/businessLogic/messageParsing";
...
describe("parseHfpPulsarMessage", () => {
  test("Parse HFP message with payload stop 'null'", () => {
    ...
    const hfpDataNoPayloadStop: NonNullableFields<Required<hfp.IData>> =
      structuredClone(hfpData);
    delete hfpDataNoPayloadStop.payload.stop;
    const expectedOutput: HfpInboxQueueMessage = {
      messageId: mockMessageId(1),
      eventTimestamp,
      uniqueVehicleId: "0018/01003",
      type: "hfp",
      data: hfpDataNoPayloadStop,
      vehicleJourneyId: {
        operatingDay: "2023-10-23",
        startTime: "15:56",
        route: "5520",
        direction: 2,
      },
    ...
    expect(parseHfpPulsarMessage(logger, input)).toEqual(expectedOutput);
  });
});
