...
import {
  keepExpandingAndCombiningAndSending,
  keepUpdatingTripDetails,
} from "./matcher";
import {
  createPulsarClient,
...
(async () => {
  ...
  try {
    ...
    try {
      ...
      const { updateTripDetails, expandWithTripDetails } = createMapper(logger);
      ...
      const promises = [
        keepUpdatingTripDetails(hfpConsumer, updateTripDetails),
        keepExpandingAndCombiningAndSending(
          producer,
          partialApcConsumer,
