...
Compose. The services are described in ./docker-compose.yml.
...
> docker-compose --file ./docker-compose-other-services.yml --file ./docker-compose-this-service.yml logs --no-log-prefix --timestamps collect-apc-results

3. Observe Pulsar state

If you wish to see the pulsar-manager UI when the stack is running, wait for a
while after the services have been started, open http://localhost:9527 in your
...
chmod go+r ./config/vehicle-db/* &&
	docker-compose \
		...
		--file ./docker-compose-this-service.yml \
		down &&
	docker-compose \
		--file ./docker-compose-other-services.yml \
		--file ./docker-compose-this-service.yml \
